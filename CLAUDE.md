...
5. **Scoped database roles, not one service key.** The Cloudflare Worker uses `cs_worker` (INSERT into consent_events and tracker_observations only). The delivery Edge Function uses `cs_delivery`. Edge Functions use `cs_orchestrator` via the hosted Supabase pool. The customer-app Next.js runtime uses two LOGIN roles via the Supavisor transaction pooler + `postgres.js`: `cs_api` for `/api/v1/*` (zero table privileges, EXECUTE limited to the whitelisted v1 SECURITY DEFINER RPCs (19 as of ADR-1005 Sprint 5.1; see `supabase/migrations/20260801000008…` and subsequent `*_grants.sql`), every one fenced by `assert_api_key_binding(p_key_id, p_org_id)` before any tenant-visible work — ADR-1009) and `cs_orchestrator` for the signup-intake + invitation-dispatch + lookup-invitation + internal/invites routes (ADR-1013). Direct-Postgres is mandatory for both Next.js roles because Supabase is rotating the legacy HS256 signing secret out — any HS256-signed scoped-role JWT stops working once revoked. As of ADR-1013 close-out, the Next.js runtime is fully off HS256 (all six callers — signup-intake, invitation-dispatch, dispatch helper, lookup-invitation, internal/invites, run-probes — use `csOrchestrator()` or `csApi()`). Never use SUPABASE_SERVICE_ROLE_KEY in running customer-app code — it is for migrations only. A CI grep gate (`scripts/check-no-service-role-in-customer-app.ts`, wired into `app/` prelint + prebuild) refuses any reintroduction. **Carve-out (ADR-0045):** admin Route Handlers under `admin/src/app/api/admin/*` may use the service role **solely** for `auth.admin.*` operations (user create / update / delete), because Supabase exposes those APIs only to the service role. Every such handler MUST (a) live behind the admin proxy (`is_admin` + AAL2), and (b) call an `admin.*` SECURITY DEFINER RPC that runs `admin.require_admin('platform_operator')` **before** the `auth.admin.*` call. Non-auth work in those handlers (reads, joins, user-visible data) uses the authed `cs_admin` client like every other admin surface.
