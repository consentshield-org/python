...
export async function createWorker(opts: HarnessOptions): Promise<{
  ...
  const mf = new Miniflare({
    ...
    bindings: {
      ...
      // ADR-1010 Phase 4 — production no longer sets SUPABASE_WORKER_KEY
      // (the wrangler secret was deleted at cutover). The harness still
      // sets a mock value because the REST fallback helpers in the
      // Worker source reference it; Hyperdrive (env.HYPERDRIVE) is
      // intentionally NOT bound here so the REST mock-server path runs.
