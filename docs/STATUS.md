...
- Test suite: **69 / 69 passing** on every build (39 RLS isolation + 5 URL-path RLS + 4 rate-limit fallback + 7 SLA-timer + 10 Worker events + 4 Worker banner). Live Supabase round trips where the test requires it; Worker tests run entirely in Miniflare with mocked outbound Supabase.
