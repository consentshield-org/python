...
### Phase 2 — Self-hosted Uptime Kuma at `status.consentshield.in` (ADR-1018 Phase 2, In Progress 2026-04-26)

Pivots the public surface from in-product `/status` route (Phase 1) to a dedicated open-source Kuma instance reachable directly at `status.consentshield.in`. **Better Stack was selected on 2026-04-25 and abandoned on 2026-04-26** after free-tier blockers (custom domain, subscribers, password protection all paid-tier-gated) made the trajectory cost-noisy for the audience size; Kuma replaces it with no SaaS spend and no artificial limits. ADR-1018 Phase 2's Supersession note records both the BS-→-Kuma pivot reasoning and the Better Stack teardown record.

| Layer | Surface (post-Phase-2) |
|---|---|
| Hosting | Uptime Kuma (open source) on Railway, region `asia-southeast1` (Singapore). Operator runs the instance; `info@consentshield.in` is the Kuma admin login. |
| Cost | Kuma is free; Railway hosting at low-single-digit USD/month or under existing Railway allowance. No tier upgrades, no per-monitor billing. |
| Resources | 7 monitors mirroring the marketing wireframe at `marketing/src/app/docs/status/page.mdx` — REST API v1, Worker event ingestion, Rights-request portal, Dashboard, Admin console, Deletion-connector dispatch, Notification dispatch. Mix of Kuma HTTP-pull monitors (anything with an unauthenticated probe URL) and Kuma push monitors (subsystems that emit a heartbeat from inside the application code: Worker on each successful HMAC-verified write, Edge Function on each successful sweep, etc.). |
| Status page | Kuma's native status page surface, branded ConsentShield (logo + palette + footer). Slug `consentshield`; custom domain `status.consentshield.in` configured inside Kuma. No Vercel CDN in path. |
| DNS (Phase 2) | `status.consentshield.in` repointed at the Railway-hosted Kuma instance on 2026-04-26. The Phase-1 Vercel alias on the `app` project is dead code; Sprint 2.7 retires it. |
| Subscriber notifications | Email subscriber form (Kuma SMTP via Resend's `smtp.resend.com:465` reusing `RESEND_API_KEY`) + Slack on `#consentshield-status` for sev1 / sev2 + native RSS feed at `https://status.consentshield.in/api/rss/consentshield` + optional partner webhook channel. |
| Incident-comms | Kuma's native incident system: sev1 / sev2 / sev3 severity matrix; `investigating` → `identified` → `monitoring` → `resolved` lifecycle; sev1 / sev2 post-mortems published within 14 business days. Templates configured in Kuma settings. |
| Auth surface | `KUMA_API_KEY` lives in repo-root `.secrets` (gitignored). Used for Kuma's `/metrics` Prometheus scrape (HTTP Basic auth, password-only) and per-monitor push-URL bearers. **Not** a control-plane key — Kuma's REST surface is dashboard-only for monitor management. |
