...
**Sprint:** Phase 2, Sprint 2.1 (Python SDK foundation)

### Added
- `packages/python-client/` — new package layout. `pyproject.toml` with `hatchling` build backend; py.typed marker per PEP 561; Python ≥ 3.9 baseline; `httpx >=0.27,<1.0` runtime dep; optional `[test]` extras (`pytest`, `pytest-httpx`, `respx`, `pytest-cov`); optional `[dev]` extras (`mypy`, `ruff`); strict tool configs (pytest, coverage `fail_under=80`, mypy `strict=true`, ruff `target-version=py39`).
- **`consentshield/errors.py`** — five-class hierarchy mirroring the Node SDK 1:1: `ConsentShieldError` base + `ConsentShieldApiError` (with `status` + `problem` fields) + `ConsentShieldNetworkError` + `ConsentShieldTimeoutError` (never retried) + `ConsentVerifyError` (compliance-critical wrapper). Every error carries optional `trace_id`. Empty-string `detail` falls back to `title` then `HTTP <status>` (matches Node behaviour).
- **`consentshield/types.py`** — wire-format types as TypedDicts, exactly mirroring the server contract: `VerifyEnvelope`, `VerifyBatchEnvelope`, `VerifyBatchResultRow`, `OpenFailureEnvelope`, `RecordEnvelope`, `RecordedArtefact`, `RevokeEnvelope`, `ArtefactListItem`, `ArtefactListEnvelope`, `ArtefactDetail`, `ArtefactRevocation`, `EventListItem`, `EventListEnvelope`, `DeletionTriggerEnvelope`, `DeletionReceiptRow`, `DeletionReceiptsEnvelope`, `RightsRequestCreatedEnvelope`, `RightsRequestItem`, `RightsRequestListEnvelope`, `AuditLogItem`, `AuditLogEnvelope`. Literal aliases for `VerifyStatus`, `IdentifierType`, `DeletionReason`, `RightsRequestType`, `RightsRequestStatus`, `RightsCapturedVia`, `OpenFailureCause`. snake_case fields preserved verbatim (server contract).
- **`consentshield/_config.py`** — shared constructor validation. `cs_live_` prefix check (case-sensitive), positive `timeout_ms` (rejects `0`, negative, non-int, `bool`), non-negative `max_retries` (rejects `-1`, non-int, `bool`), bool-only `fail_open`. `CONSENT_VERIFY_FAIL_OPEN=true|1` env honoured when option absent; explicit option overrides env. Trims trailing slashes off `base_url`.
- **`consentshield/_http.py`** — sync `HttpClient` + async `AsyncHttpClient` via `httpx.Client` / `httpx.AsyncClient`. 2-second default timeout via `httpx.Timeout`. Exponential backoff (100/400/1600 ms) on 5xx + transport errors up to `max_retries`. **Never** retries 4xx (caller bug). **Never** retries timeouts (compounds latency past compliance budget). RFC 7807 problem-document parsing onto `error.problem`. `X-CS-Trace-Id` round-trip. Both clients accept a caller-supplied `httpx.Client` / `httpx.AsyncClient` for testing + custom transports (e.g. `httpx.MockTransport`).
- **`consentshield/_builders.py`** — synchronous request construction shared between sync + async clients. Each `build_*_request(...)` validates input, constructs the snake_case body/query at the network boundary, returns an `HttpRequest`. URL-encodes path params (defends against artefact ids with `/`/`#`/`&`/`?`). Compliance gates fire BEFORE network: `verify_batch` empty → `ValueError`, > 10 000 → `ValueError` matching server cap, exactly 10 000 allowed at boundary, non-string entry → `TypeError`; `record_consent` empty `purpose_definition_ids` → `ValueError`; `trigger_deletion` `purpose_codes` REQUIRED when `reason='consent_revoked'` → `TypeError`; whitelisted enums for actor types / reasons / rights types / statuses / captured_via.
- **`consentshield/_verify.py`** — `decide_failure_outcome(err, fail_open)` encodes the **non-negotiable compliance contract**: 4xx ALWAYS raises (`_Rethrow`), timeout/network/5xx + fail_open=False → `_Rethrow(ConsentVerifyError)`, fail_open=True → `_Open(OpenFailureEnvelope)` with `cause` discriminator (`'timeout' | 'network' | 'server_error'`) + `trace_id`. `is_open_failure(result)` ergonomic type guard re-exported from the package root.
- **`consentshield/client.py`** — sync `ConsentShieldClient`. 14 methods + 5 cursor-iterating generators (`iterate_artefacts`, `iterate_events`, `iterate_deletion_receipts`, `iterate_rights_requests`, `iterate_audit_log`). Default `on_fail_open` callback: `logging.warning('[consentshield] fail-open verify override ...')` with structured key/value pairs. Sync throw inside callback → caught + emitted via stderr; never breaks the verify call site. Context-manager support (`with ConsentShieldClient(...) as client:`).
- **`consentshield/async_client.py`** — async `AsyncConsentShieldClient`. 100% method parity; every method returns `Awaitable[T]` and cursor iterators are `AsyncIterator[T]`. `on_fail_open` accepts both sync callables AND coroutines; awaitable returns scheduled fire-and-forget on the running event loop (or driven via `asyncio.run` when no loop). Sync throws + async rejections both swallowed with stderr log. Async-context-manager support (`async with AsyncConsentShieldClient(...) as client:`).
- **`packages/python-client/LICENSE.md`** — proprietary text per CLAUDE.md authorship rules.
- **`packages/python-client/README.md`** — quickstart for both sync + async, full 14-method matrix grouped by concern (consent / listing+iteration / deletion+rights), explicit fail-open behaviour table, error model summary.
- **Test suite (10 files, 119 tests):** `test_errors.py` (7), `test_client.py` (14), `test_http.py` (16), `test_verify.py` (12), `test_verify_batch.py` (12), `test_methods.py` (15), `test_async_client.py` (7), `test_fail_open_callback.py` (7), `test_iterators_and_validators.py` (22), `conftest.py` (shared `json_response` / `problem_response` helpers + fixtures). Uses `httpx.MockTransport` for stubbing — no network round-trips. Coverage **88.54%** (statements/branches/functions/lines combined) — above the 80% gate enforced via `pyproject.toml` `[tool.coverage.report] fail_under=80`.

### Architecture Changes
- **Spec amendment for Sprint 2.1 — sync + async client parity.** Original ADR text said "`httpx` for HTTP (supports async + sync)" without committing to a client shape. After Phase-1's Node-SDK pattern landed, the natural choice for Python is **two clients** matching the convention every major Python SDK uses (httpx, openai, anthropic, redis-py). User-confirmed before Sprint 2.1 work started. Doubles the implementation surface; halves the integration friction for downstream callers.
- **Shared core keeps the duplication minimal.** Three shared modules (`_config.py`, `_builders.py`, `_verify.py`) hold all the validation + body-shape + failure-decision logic; the sync + async clients are essentially thin transport-specific shells around the shared core. Each method is one `req = B.build_X(...)` call followed by `self._http.request(req)` (or `await self._http.request(req)` on async).
- **The non-negotiable compliance contract is preserved across both clients.** `decide_failure_outcome` returns dataclasses (`_Rethrow` / `_Open`) rather than raising or returning directly; the sync verify raises/returns and the async verify does the same with appropriate `await` semantics. Both invoke `on_fail_open` exactly once when the open envelope is built. Same 4xx-always-raises contract; same fail-CLOSED default; same env override.
- **Tests use `httpx.MockTransport` rather than `pytest-httpx` or live servers.** `MockTransport` is httpx-native, has zero setup overhead, and lets tests assert on the raw `httpx.Request` (URL, headers, body bytes) directly. Conftest helpers `json_response` + `problem_response` build well-formed responses including JSON `null` bodies (httpx's `json=None` kwarg skips serialisation entirely; tests serialise via `content=` to get the literal `null` body for `get_artefact` returns-None testing).
- **Spec amendment for Sprint 2.1 — `pip publish` deferred to Sprint 2.2.** SDK is publish-ready (pyproject.toml validated, mypy strict clean, 88.54% coverage). `python -m build` + `twine upload` + the Django/Flask/FastAPI integration examples ship in Sprint 2.2 (mirrors Sprint 1.4's deferral pattern for the Node SDK).

### Tested
- [x] `cd packages/python-client && .venv/bin/pytest -q --cov` — **119 passed**; coverage **88.54%** total (`Required test coverage of 80.0% reached`) — PASS
- [x] `cd packages/python-client && .venv/bin/mypy src/consentshield` — `Success: no issues found in 9 source files` — PASS
- [x] All cursor iterator helpers exercised end-to-end (sync + async, 8 tests across 4 method groups walking 2-page sequences).
- [x] Compliance-load-bearing rules verified: 4xx-always-raises sweep across 422/403/404 with `fail_open=True`; fail-CLOSED default raises `ConsentVerifyError`; fail-OPEN returns `OpenFailureEnvelope` with `cause` discriminator (timeout/network/server_error); `verify_batch` boundary at exactly 10 000 allowed, 10 001 raises synchronously.
...
**ADR:** ADR-1006 — Developer Experience: Client Libraries + OpenAPI + CI Drift Check
**Sprint:** Phase 1, Sprint 1.2 (verify + verifyBatch — the compliance-load-bearing methods)

### Added
- `packages/node-client/src/types.ts` — wire-format types mirroring `app/src/lib/consent/verify.ts` exactly. `VerifyStatus` (`granted | revoked | expired | never_consented`), `IdentifierType` literal union (`email | phone | pan | aadhaar | custom`), `VerifyEnvelope` (§5.1 single-identifier response), `VerifyBatchEnvelope` (results array preserves input order), `VerifyBatchResultRow`, **`OpenFailureEnvelope`** (`{ status: 'open_failure', reason, cause: 'timeout' | 'network' | 'server_error', traceId? }`). snake_case field names preserved on response shapes (server contract); SDK input shapes use camelCase per JS/TS convention.
- `packages/node-client/src/verify.ts` — `verify()` + `verifyBatch()` core. Implements the **non-negotiable compliance contract** at the `decideFailureOutcome()` helper:
  - **4xx ALWAYS throws** `ConsentShieldApiError` regardless of `failOpen` (caller bug / scope / 422 / 404 / 413 — failOpen MUST NEVER mask a real validation/scope error).
  ...
  - **timeout / network / 5xx + `failOpen=true`** → returns `OpenFailureEnvelope` with `cause` discriminator + `traceId` from the failed-request response header. Sprint 1.3 wires the automatic POST to `/v1/audit` for the override; Sprint 1.2 ships the shape only.
- `client.verify({ propertyId, dataPrincipalIdentifier, identifierType, purposeCode, traceId?, signal? })` — GET `/v1/consent/verify` with snake_case query string composition at the network boundary. Returns `VerifyEnvelope | OpenFailureEnvelope`.
- `client.verifyBatch({ propertyId, identifierType, purposeCode, identifiers, traceId?, signal? })` — POST `/v1/consent/verify/batch` with snake_case body. Returns `VerifyBatchEnvelope | OpenFailureEnvelope`.
- **Client-side gates BEFORE any network call** on `verifyBatch`: empty array → `RangeError` synchronously; > 10 000 → `RangeError` synchronously matching the server's `MAX_IDENTIFIERS` cap (saves the 413 round-trip + exact match at boundary 10 000); non-array `identifiers` → `TypeError`; non-string entry → `TypeError`; empty-string entry → `TypeError`; missing required scalar → `TypeError`. None of these gates trigger fetch.
- `isOpenFailure(result)` — ergonomic type guard so callers can branch without a `.status === 'open_failure'` string check. Re-exported from `index.ts`.
- `tests/verify.test.ts` (15 cases) — happy path returns envelope verbatim; camelCase → snake_case query; Authorization Bearer; traceId forward; synchronous validation rejects each missing/empty required field; **fail-closed default** throws `ConsentVerifyError` (not raw API error) on 5xx + transport error; **fail-open opt-in** returns `OpenFailureEnvelope` on 5xx + transport + timeout (with correct `cause` discriminator); **4xx ALWAYS throws** even when failOpen=true (sweep across 422 / 403 / 404).
- `tests/verify-batch.test.ts` (15 cases) — POST shape (snake_case body, Content-Type) / input-order preserved in results; client-side gates trigger before fetch (empty / > 10 000 / non-array / non-string entry / empty entry / missing required); fail-closed/open + 4xx-never-opens (422 + 413).

### Changed
- `packages/node-client/src/client.ts` — `ConsentShieldClient` gains `verify()` + `verifyBatch()` methods. JSDoc on each spells out the behaviour table (200 / 4xx / 5xx-or-timeout-or-network × failOpen=true|false) so the contract is visible at the call site.
- `packages/node-client/src/index.ts` — re-exports `isOpenFailure` + `VerifyInput` + `VerifyBatchInput` + the new `types.ts` shapes (`IdentifierType`, `OpenFailureEnvelope`, `VerifyBatchEnvelope`, `VerifyBatchResultRow`, `VerifyEnvelope`, `VerifyStatus`).

### Architecture Changes
- **The 4xx-always-throws rule is the non-negotiable load-bearing piece.** A failOpen flag must NEVER silence a real validation, scope, or auth error — those are caller bugs that need to surface. Tested via three sweep cases (422 / 403 / 404 — validation, scope, not-found) all under `failOpen=true`. Documented in the ADR §Sprint 1.2 + the JSDoc behaviour table on `client.verify()` / `client.verifyBatch()`.
- **`OpenFailureEnvelope` carries a `cause` discriminator** (`'timeout' | 'network' | 'server_error'`) so the audit-trail entry in Sprint 1.3 can record WHY the override happened, not just THAT it happened. The `traceId` from the failed-request response header round-trips through too — server-side log correlation stays one grep away even on the failure path.
- **Client-side `verifyBatch` cap matches the server's `MAX_IDENTIFIERS = 10000` exactly.** Boundary test confirms 10 000 is allowed; 10 001 throws synchronously. If the server cap ever changes, the SDK constant must move with it (a code-search grep for `MAX_BATCH_IDENTIFIERS` is the safety net).
...
**Sprint:** Phase 4, Sprint 4.2 follow-up

### Added
- `app/stryker.sigv4.conf.mjs` — Stryker 9.6.1 with vitest-runner + typescript-checker plugins; `coverageAnalysis: 'perTest'`; HTML + JSON reporters under `app/reports/mutation/sigv4/`. Mutate scope: `src/lib/storage/sigv4.ts` (entire file). **Carve-out break threshold of 75** (vs the standard 80) — see Architecture Changes.
- `app/tsconfig.stryker.sigv4.json` — checker-only tsconfig scoped to sigv4.ts (mirrors the Sprint 4.2 / 4.3 pattern; excludes `tests/` to prevent pre-existing lax-mode test-file typing breaking the checker init).
- `app/package.json` — new script `test:mutation:sigv4`.
- `scripts/capture-sigv4-vectors.ts` — emits pinned AWS sigv4 signatures for `presignGet`, `putObject` (with metadata headers), `deleteObject`, `probeHead/Get/Delete/List`. Frozen clock: `Date.UTC(2026, 0, 15, 8, 0, 0)`. Re-run only when sigv4 implementation changes intentionally.
- `app/tests/storage/sigv4.test.ts` — extended from 12 to 28 cases. New `describe('pinned vectors (ADR-1014 Phase-4 follow-up)')` block uses `vi.useFakeTimers` + `vi.setSystemTime(FROZEN)` to lock time, then asserts EXACT signature hex for every signed code path. Includes pinned tests for `formatAmzDate`, `deriveSigningKey` (32-byte chain), `canonicalUriFor`, `sha256Hex`, `presignGet`, `putObject` (signature + metadata-headers actually-written assertion to kill the L110 for-loop-empty mutant), `deleteObject` (with 5xx body excerpt + 404 idempotent + 204 success branches), `probeHeadObject`, `probeGetObject`, `probeDeleteObject`, `probeListObjectsV2` (bucket-root with `?list-type=2`), and `sha256Hex` Buffer + Uint8Array branches.

### Changed
- `scripts/run-mutation-suite.ts` — `MODULES` array now includes `id: 'sigv4'` with `breakThreshold: 75`. Aggregate driver runs and reports four modules.
- `testing/src/data/types.ts` — `ModuleMutationScore.id` widened from `'worker' | 'delivery' | 'v1'` to include `'sigv4'`.
- `testing/src/data/runs.ts` — new published run `06EW0PT8M5XKDV6N9R3FB72JKQ` / commit `0beb495ab1cd` with the four-module breakdown (worker 91.07 / delivery 95.65 / v1 100 / sigv4 78.26 → aggregate 91.24).
- `.gitignore` — `app/.stryker-tmp-sigv4/`.

### Architecture Changes
- **Carve-out break threshold of 75 for sigv4.** The hand-rolled AWS sigv4 signer carries a documented ~32-mutant equivalent floor that no test can kill without modifying production code (which Rule 13 forbids). Each equivalent is enumerated in ADR-1014 §"Sprint 4.2 follow-up — sigv4 mutation kill-set":
  - **L60 + L70 + L71 (16):** redundant `metaPairs.sort` / `allHeaders.sort` — the metadata key prefix `x-amz-meta-` guarantees alphabetical order of the merged list, so the final sort is dead code for ALL valid inputs.
  - **L321-322 (8):** presignGet's URLSearchParams sort — the code adds params in alphabetical order by construction (X-Amz-Algorithm < Credential < Date < Expires < SignedHeaders), so the comparator is a no-op.
  - **L378 (3):** sha256Hex's input-type ternary — Node's `Hash.update` accepts string | Buffer | Uint8Array natively and produces the same digest, so the type-check branches are equivalent.
  ...
  - **L243 (2):** `keyForPath === ''` ternary — both branches produce `/bucket/` for empty key, and both produce the same canonicalUri for non-empty keys.
  - **L290 (1):** `try { await resp.arrayBuffer() }` drain block — a connection-release optimization, not a behaviour gate.
  - **L222:52 (1):** `key: ''` literal in `signedProbeRequest('GET', { ...opts, key: '' }, ...)` — `opts.key` is unused inside `signedProbeRequest` (which uses the 4th arg `keyForPath`).
- The "covered" mutation score (which excludes NoCoverage and the equivalent floor) sits at **83.24%** — the test suite IS doing the discriminatory work; the threshold reflects what's *killable* without violating Rule 13. The 90% high target stays — any future improvement (e.g. removing the redundant L60 sort as a code-cleanup ADR) is welcome.
- **NoCoverage instrumentation behaviour.** 11 NoCoverage mutants sit at top-level constants (`SERVICE = 's3'` at L40 etc.) that Stryker's `coverageAnalysis: 'perTest'` does not attribute to test runs because they evaluate at module-load time, not inside a test function body. This is a Stryker instrumentation limitation, not a test gap.

### Tested
- [x] `cd app && bun run test tests/storage/sigv4.test.ts` — 28 passed (1 file) — PASS
- [x] `cd app && bun run test:mutation:sigv4` — **78.26% mutation score** (covered 83.24%); 144 killed, 0 timeouts, 29 survived (all documented equivalent), 11 NoCoverage (top-level constants), 47 typecheck-rejected. Above the carved-out `break: 75` threshold.
- [x] `bun run test:mutation:report-only` — full aggregate gate: Worker 91.07 / Delivery 95.65 / v1 100.00 / sigv4 78.26 — all pass their thresholds — PASS.
- [x] `cd testing && bun run build` — testing site re-builds with the new published run + `4.2-followup` sprint route — PASS.
