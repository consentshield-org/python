...
- **`marketing/src/app/docs/sdks/go/page.mdx`** — `github.com/SAnegondhi/consentshield-go` landing page. Install + Go 1.22+ note + zero-runtime-deps emphasis + Go-module-proxy release model. Quickstart, configuration knobs, full method matrix, compliance contract with `errors.As`-discrimination snippet, paginator pattern (`Next(ctx) bool` + `Page() []T` + `Err() error`), trace-id round-trip via `Error` interface, `Config.HTTPClient` swap for tests / mTLS / custom transport, examples reference (net/http + gin).
