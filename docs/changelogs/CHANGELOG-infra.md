...
### Added
- `scripts/run-mutation-suite.ts` — aggregate driver. Runs the three Phase-4 Stryker configurations (`worker/stryker.conf.mjs`, `app/stryker.delivery.conf.mjs`, `app/stryker.v1.conf.mjs`) sequentially via `bun run test:mutation:{,delivery,v1}`. Parses each `mutation.json`, computes per-module score `(killed + timeout) / (killed + survived + timeout + noCoverage)`, renders a single human-readable summary table, writes `reports/mutation/summary.json`, exits 1 if any module is below its `break: 80` threshold. Flags: `--module worker|delivery|v1` (single module), `--skip-runs` / `--report-only` (parses existing reports without re-running Stryker).
- `package.json` (root) — new scripts `test:mutation` (full aggregate) + `test:mutation:report-only` (parse-only).
- `.github/workflows/mutation.yml` — nightly Stryker workflow. Schedule: 04:30 UTC. Manual trigger: `gh workflow run mutation.yml`. `timeout-minutes: 25`. Bun setup → `bun install --frozen-lockfile` → `bun run test:mutation` (with `continue-on-error` so artefacts upload even on failure) → upload three module HTML reports + `reports/mutation/summary.json` as the `mutation-html-reports` artefact (30-day retention) → re-assert the gate (fail the build if Stryker step failed). PR runs are NOT gated — Stryker's per-mutant subprocess fan-out makes per-PR execution expensive for low marginal value (the same kills surface overnight); active feature work uses the per-module commands locally.
- `testing/src/data/types.ts` — extended `PublishedRun` with `mutation: ModuleMutationScore[] | null`. Each entry: `id` (constrained to `'worker' | 'delivery' | 'v1'`), `label`, `score` (0-100, one decimal), `killed`, `survived`, `equivalent` (subset of survived classified as no-observable-behaviour-change), `noCoverage`, `timeout`, `sprint`. The pre-existing `mutationScore: number | null` field stays as the aggregate (mean of module scores) for the list view.
- `testing/src/data/runs.ts` — new published run at the head: `runId 06EW0M4Q9C2P3S5SVJ6X8Y4F7N` / commit `55d6275a8e9c` / branch `main` / aggregate score **95.57** (mean of 91.07 + 95.65 + 100.00) with the three per-module entries fully populated (Worker 50/5/5/0/1, Delivery 66/3/3/0/0, v1 25/0/0/0/0). Tally counts the underlying unit-test pool Stryker ran each mutant against (49 + 197 + 55 = 301 tests). Sprint tags `['4.1', '4.2', '4.3', '4.4']`; phase `[4]`. Notes line summarises the equivalent-mutant total (8 across the three modules) + the sigv4 deferral.
- `testing/src/app/runs/[runId]/page.tsx` — new "Mutation testing breakdown" section between coverage block and notes. Per-module table with columns: Module / Sprint / Score (colour-coded: ≥90 emerald, 80-90 amber, <80 red) / Killed / Survived (red when survived > equivalent, slate otherwise) / Equivalent / Timeout. Section is hidden entirely when `run.mutation === null` so Phase 5 reproduction runs (no Stryker layer) don't show an empty table.

### Architecture Changes
- **Spec amendment for Sprint 4.4 publication path.** The original deliverable wording was "Nightly Stryker run publishes HTML to `testing.consentshield.in/runs/<sha>/mutation/`". That predates Sprint 5.3's "no upload pipeline; everything is git-tracked" decision (the testing site is fully static + has no R2 SDK or external write surface). Sprint 4.4 reconciles by publishing per-module **scores + counts** as structured fields on the public `PublishedRun` schema, while keeping the per-mutant **HTML reports** as 30-day GitHub Actions artefacts. Reviewers who want per-mutant detail either run the suite locally (`bun run test:mutation`) or download the artefact from the workflow run. The published score table on `testing.consentshield.in/runs/<runId>` carries the killed/survived/equivalent breakdown in human-readable form. Captured in the ADR Sprint 4.4 body.
- **Negative-control verification of the gate.** A scratch script that flips 20 `Killed` entries to `Survived` in the v1 mutation.json then runs `bun run test:mutation:report-only` confirmed the aggregate driver correctly reports `❌ 1 module(s) under their break threshold: v1 (56% < 80%)` and exits with code 1. The canonical mutation.json was regenerated by re-running `bun run test:mutation:v1` to restore the 100% baseline.

### Tested
- [x] `bun run test:mutation` (full aggregate) — Worker 91.07, Delivery 95.65, v1 100.00 — all ≥80 — PASS
- [x] `bun run test:mutation:report-only` — same outcome from cached `mutation.json` files — PASS
- [x] Negative control — flip 20 Killed → Survived in v1 → driver exits 1 with `❌ v1 (56% < 80%)` — PASS
- [x] `cd testing && bun run build` — 11 prerendered pages including the new Phase-4 run with the colour-coded mutation table; site-wide build clean — PASS
- [x] `cd marketing && bun run build` — 24 static `/docs/*` routes (up from 23) including `/docs/test-verification/mutation-testing` listed as `○ (Static)`; nav + Cmd-K palette wiring verified — PASS
