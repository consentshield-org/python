...
| ADR-1010 | Cloudflare Worker — scoped-role migration off HS256 JWT (Hyperdrive / direct Postgres; Supabase is rotating legacy HS256 signing secret → `SUPABASE_WORKER_KEY` is on a kill-timer). Sprint 2.1 complete 2026-04-22 — cs_worker password rotated, pooler URL wired, BYPASSRLS granted, 11/11 cs-worker-role tests PASS. Rule-5 runtime role guard in `worker/src/role-guard.ts` same day — 13/13 guard tests + 33/33 Miniflare tests PASS. Phase 1 Sprint 1.1 scaffold shipped 2026-04-22 (`/v1/_cs_api_probe` + three mechanism scaffolds; 39/39 worker tests PASS). Phase 1 Sprint 1.2 complete 2026-04-22 — operator provisioned `cs-worker-hyperdrive` Hyperdrive config; `[[hyperdrive]]` binding in wrangler.toml; Worker redeployed; probes confirm REST baseline p50 274ms vs Hyperdrive-binding reachable. Mechanism = Hyperdrive. **Phase 3 Sprint 3.1 complete 2026-04-22** — banner/origin/signatures dual-path over postgres.js via env.HYPERDRIVE (single Rule-16 carve-out, exact-pinned); `snippet_last_seen_at` fire-and-forget ported; Miniflare harness keeps REST fallback path green (39/39 PASS); new Hyperdrive-reads integration test 5/5 PASS against real cs_worker DSN; Worker redeployed (version 19896a12...). Prod smoke test deferred until Sprint 3.2 retires SUPABASE_WORKER_KEY + role guard. | In Progress | 2026-04-21 | 4 | 6 |
