...
| ADR-1010 | Cloudflare Worker — scoped-role migration off HS256 JWT to Hyperdrive (Supabase HS256 kill-timer). All four phases shipped 2026-04-22..23. Phase 1: prototype scaffold + Hyperdrive mechanism decision. Phase 2: cs_worker password rotated + BYPASSRLS + role-guard. Phase 3 Sprint 3.1: read paths (banner/origin/signatures) on postgres.js via env.HYPERDRIVE (Rule-16 carve-out: postgres@3.4.9 exact-pinned). Phase 3 Sprint 3.2: write paths (events/observations/worker-errors) — discovered + fixed `${JSON.stringify}::jsonb` double-encoding bug, switched to `sql.json()`. Phase 4: deleted role guard + prototypes + probe route + SUPABASE_WORKER_KEY wrangler secret. Live banner.js smoke 5/5 HTTP=200 (cold 2.9s, warm 60-100ms). 9 integration tests PASS (5 reads + 4 writes); 20/20 Miniflare unchanged. Sprints 4.2 (request-scoped client + ctx.waitUntil cleanup) + 4.3 (strip REST fallback) tracked as follow-ups on `admin.ops_readiness_flags` — neither blocking. | Completed | 2026-04-21 | 4 | 6 |
