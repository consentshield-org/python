...
| ADR-1004 | v2 Whitepaper Phase 4 — Statutory retention (Regulatory Exemption Engine) + material-change re-consent + silent-failure detection; Phase 1 Sprints 1.1-1.4 shipped 2026-04-22 (schema + BFSI/Healthcare seeds + deletion orchestrator integration; Sprints 1.5 dashboard + 1.6 legal review + Phase 2 notices + Phase 3 orphan metric pending) | In Progress | 2026-04-19 | 3 | 9 |
| ADR-1005 | v2 Whitepaper Phase 5 — Operations maturity (webhook reference, test_delete, support model, status page, rights API, non-email channels); Sprint 5.1 shipped 2026-04-22 — `POST /v1/rights/requests` + `GET /v1/rights/requests` with identity attestation + captured_via channel + created_via_api audit event | In Progress | 2026-04-19 | 6 | 10 |
...
| ADR-1010 | Cloudflare Worker — scoped-role migration off HS256 JWT + Rule-5 remediation (discovered mid-flight: `SUPABASE_WORKER_KEY` = service-role `sb_secret_*`). Sprint 2.1 complete 2026-04-22 — cs_worker password rotated, pooler URL wired to .env.local/.env.local@app, BYPASSRLS granted, 11/11 cs-worker-role tests PASS. Phase 3 Worker source rewrite unblocked; Phase 1 Hyperdrive prototype still scoped. | In Progress | 2026-04-21 | 4 | 6 |
